...
**Question:** Does ones' perception of their weight impact their GPA?

**Hypotheses:** We will rely on the standard hypotheses for a Kruskal-Wallis Rank Sum test.
...
*(Click on the tabs for additional plots)*
...
Lastly, a numerical summary of GPA for each of the factor levels of SPW (`self_perception_weight`) will further confirm the similarity in GPA scores even when sorted by factor level.
...
```{r boxplot_summaryGPA, fig_height=5}
ggplot() +
  # boxplots
  geom_boxplot(
    data = food2,
  ...
  geom_line(
    data = fvs,
    aes(x = SPW, y = mean, group = 1, color = "Mean"), linetype = "dashed", linewidth = 1
  ) +
  # geom_line(
  #   data = fvs,
  #   aes(x = SPW, y = median, group = 1, color = "Median"), linetype = "solid", linewidth = 1, alpha = .7
  # ) +
